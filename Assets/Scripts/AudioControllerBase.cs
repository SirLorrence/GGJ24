...
public class AudioControllerBase : MonoBehaviour
{
    ...
    public void Play(AudioClip audioClip, Vector3? worldPos = null, float? volume = null)
    {
        if (audioClip == null)
            return;

        if (worldPos != null)
            PlayInWorldSpace(audioClip, worldPos, volume);
        else
            Play(GetTrack(audioClip.length, null), audioClip);
    }

    void PlayInWorldSpace(AudioClip audioClip, Vector3? worldPos, float? volume = null)
    {
        var track = GetTrack(audioClip.length, worldPos);
        if (volume != null) track.volume = volume.Value;
    ...
    AudioSource GetTrack(float length, Vector3? worldPos)
    {
        ...
        if (worldPos!= null)
        {
            track.transform.position = worldPos.Value;
